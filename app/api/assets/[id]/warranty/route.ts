...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await prisma.auditLog.create({
      data: { asset_id: params.id, performed_by_user_id: session.user.id, action_type: "ASSET_WARRANTY_UPDATED", previous_value: { warranty_expiry_date: asset.warranty_expiry_date } as any, new_value: { warranty_expiry_date, warranty_notes } as any },
    });
