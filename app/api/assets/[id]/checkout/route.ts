...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const checkout = await prisma.checkoutRecord.create({
      data: { asset_id: params.id, checked_out_by_user_id: session.user.id, purpose: purpose || null, expected_return_date: expected_return_date ? new Date(expected_return_date) : null },
    });
    ...
    await prisma.auditLog.create({
      data: { asset_id: params.id, performed_by_user_id: session.user.id, action_type: "ASSET_CHECKED_OUT", new_value: { checked_out_to: session.user.name, purpose } as any },
    });
