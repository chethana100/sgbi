...
export async function POST(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await prisma.auditLog.create({
      data: { asset_id: params.id, performed_by_user_id: session.user.id, action_type: "ASSET_SERVICE_RESET", previous_value: { last_service_date: asset.last_service_date } as any, new_value: { last_service_date: new Date() } as any },
    });
