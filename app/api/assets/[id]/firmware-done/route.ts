...
export async function POST(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await prisma.auditLog.create({
      data: { asset_id: params.id, performed_by_user_id: session.user.id, action_type: "ASSET_FIRMWARE_UPDATED", previous_value: { firmware: asset.current_firmware } as any, new_value: { firmware: fw.latest_version } as any },
    });
